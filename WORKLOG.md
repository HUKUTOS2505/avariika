...
## БОЛЬНИЦА v2 — ЭТАП 1 (greybox вертикального среза) ГОТОВ 2026-06-13, ждёт прогона пользователем

Пользователь дал отмашку строить больницу по `TZ_Hospital_Map_UE5_v2.md`. По §9.2 сделан greybox ВЕРТИКАЛЬНОГО СРЕЗА (атриум + двор + западный подвальный маршрут до фазы 2), НЕ всё здание — чтобы пользователь прошёл и сказал «то/не то» до стройки остального (не повторять провал v1).
...
- ⚠️ **Свет/скриншоты:** дневной свет (movable directional + SkyAtmosphere + SkyLight) рендерится ЧЁРНЫМ в headless-PIE этой сессии (авто-экспозиция; PPV-зажим не применился — имя override-свойства не то, см. ниже). В обычном редакторе вьюпорт **Lit** покажет массинг. Игровой свет/ночь/дождь — Этап 5. Headless-скриншот массинга снять НЕ удалось (редактор в неинтерактивной станции рендерит только через активный PIE, а daylight там чёрный + нельзя выбрать, какой из 2 вьюпортов попадёт в HighResShot).

### СЛЕДУЮЩИЙ ШАГ: ждём прогон/фидбек пользователя по Этапу 1 (массинг атриума, петли, маршрут §6 до фазы 2). По «ок» — Этап 2 (вост.крыло/часовня/операционная/солярий, этажи 2-3, остальной подвал) тем же скриптом.
