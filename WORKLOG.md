...
- **Проход 4 (квест-акторы)** — `Scripts/build_hospital_quest.py`: 3 `ARepairable` с типами мини-игр в подвальных комнатах (Генератор STARTER/Welder в генераторной 5100,1200,-240; ГРЩ CURSOR/Tester 4500,1200,-215; Газовая труба VALVE/leaks 2100,1200,-180), `AExitZone` (ГАЗель) у входа, инструменты (Сварочник HEAVY, Тестер) в мастерской (3900,300), Рация, парковочная площадка `H_ParkPad` у входа (снаружи пола нет — куб-площадка, верх вровень с F1), PlayerStart перенесён на площадку (3000,-600,100, лицом в здание). Метки БЕЗ префикса GB_ (переживают пересборку каркаса). Уровень 208 акторов. Двери открыты (замки §6 — не реализованы).

### СЛЕДУЮЩИЙ ШАГ по карте
1. **Проверка прохода в PIE** (пользователь): пройти §5 от площадки → вестибюль → коридор F1 → ЛК-Б → подвал → генератор/ГРЩ/труба → назад. Капсула 34×88 должна проходить везде (подвал 240 высотой). Фидбек → фиксы геометрии.
2. Пропсы по §4 (модели meshy на заглушки), мебель.
3. Ночной свет §7 (3 фазы), дождь Niagara.
4. BP_QuestManager (стейт-машина §5), BP_Door с замками §6, ключи, канистра топлива.

**Канон пересборки карты (всё headless):** `build_hospital_greybox.py` (каркас+свет) → `build_hospital_rooms.py` (комнаты) → `build_hospital_quest.py` (квест-акторы). Порядок важен: greybox сносит все GB_ (включая GB_Part), поэтому rooms запускать ПОСЛЕ greybox; quest-акторы (не-GB) переживают, но при пересборке greybox площадку/PlayerStart восстановит сам greybox? — нет, площадка только в quest-скрипте, прогнать quest повторно.
