...
1. **Пользователь тестит в PIE** (лучше 2 игрока): чек-лист ниже. Claude ждёт фидбек и чинит баги.
2. **Модели из meshy.ai** (промпты ниже): пользователь генерит, Claude импортирует headless и назначает на акторы по меткам (`Toilet`, `Repairable_Breaker`, `Tester`).
3. Анимации (сидение на туалете, тык тестером) — пользователь, потом подключим.
...
## Очередь модулей (после текущего)

1. **Модели и вид**: импорт meshy-моделей на все заглушки, материалы.
2. **Анимации** (пользователь даёт файлы — подключаем AnimBP headless).
3. **Труба и генератор** — свои мини-игры? (обсудить: вентиль-тайминг у трубы, топливо у генератора).
4. **Монстр-слухач** — ТОЛЬКО по отмашке.
