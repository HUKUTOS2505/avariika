...
- **Паника «не уходит на свету»**: по дизайну спадает при ВКЛючённом фонаре (свет=безопасность), не от окружающего света. Добавил: спадает и стоя в `CalmRadius` прожектора (VitalsComponent). Для ночной карты — полное покрытие «свет успокаивает».
...
- **ГАЗель**: `.glb` в `RawAssets/SM_Gazelle/`, но glTF-материалы простым AssetImportTask НЕ импортируются (107 секций, цвета не подтянулись). Нужен `InterchangeGenericAssetsPipeline` с импортом материалов. Пока серая. Скрипты: `gazelle_retex.py`/`gazelle_clean.py`.
