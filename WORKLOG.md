...
- **Дальше (след. окно сборки):** earthquake-рокот слоем на взрыв; rope-скрипы в систему случайного хоррор-эмбиента; idle/огнетушитель проверить на слух; вариативность (ratchet ×3, insert ×2 через Cue/массив). Доку — `AUDIO_INVENTORY.md`. Сварка/стартер-анимация и «вентиль→заварить» — в бэклоге.
