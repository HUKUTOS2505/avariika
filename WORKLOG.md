...
**АВТОНОМ (юзер уехал, «редактор мой, стройку не трож»):** сделал диспетчер event-реакции (выше). Демо-ассеты в `HOUSE/Stairs_DEMO`/`HOUSE/Doors` оставил для решений юзера. Стройку-геометрию не трогал.
