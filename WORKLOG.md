...
1. **Пользователь сейчас играет/тестит** (2026-06-12): чек-лист ниже. **Приоритет №1 после очистки контекста — фиксы багов из его фидбека.**
...
⚠️ Пока пользователь играет, его редактор ОТКРЫТ: сборка и headless-скрипты невозможны (Live Coding/конфликт записи). Код писать можно; собирать — только когда он напишет, что закрыл редактор.

## СЛЕДУЮЩИЙ МОДУЛЬ (утверждён): «Труба + Генератор» — свои мини-игры

Пилить, пока ждём фидбек/модели. Дизайн зафиксирован:

- **Газовая труба — «вентиль»** (без инструмента): E у трубы — фиксация как у щитка; каждый тык E докручивает вентиль на ~12%; **но** если жать чаще чем раз в ~0.7 с — «срыв резьбы»: прогресс −20%, громкое шипение (MakeNoise ~1.0). Газ травит, пока не закрыто (взрыв от курения остаётся). Ритм-игра «не суетись» — в панике с трясущейся камерой жать размеренно тяжело.
- **Генератор — «стартер»** (RequiredTool=Welder остаётся): держишь E — полоска натяжения шнура растёт; отпустить в зелёном окне (~70–90%) = успешный рывок, нужно 3 успешных подряд... нет: 3 успешных всего (прогресс по трети). Перетянул до 100% или отпустил рано — «обратный удар»: −5 HP, +5 паники, шум 0.8, попытка не засчитана. Фиксация на месте, выход G.
- Реализация: оба в `ARepairable` как режимы (`ERepairMinigameType: None/Cursor/Valve/Starter` вместо bool `bMinigameRepair` — щиток=Cursor, труба=Valve, генератор=Starter), HUD рисует свой виджет на режим. Скрипт `setup_minigames.py` дополнить выставлением типов.
- После сборки: смоук, TESTING.md, WORKLOG, коммит+пуш — стандарт.

### Чек-лист теста модуля
- [ ] Бег: камеру больше НЕ дёргает вправо (паника выше порога — только мягкая дрожь).
- [ ] Туалет: E → сел НА куб, развернуло на 180°, WASD и мышь не работают; две зоны (зелёная ~20%, жёлтая ~70%), E по курсору; промах — громко; ноль — встал сам? (нет: шкала 0 → визит зачтён, лок снят, стоишь на кубе); G — встал досрочно.
- [ ] Щиток: без тестера — подсказка про инструмент; с тестером E → фиксация, мини-игра; 4 попадания = починен; промах бьёт током; 3 промаха — дуга по окружающим, «ЗАМКНУЛО (60 с)» на табличке, чинить нельзя, потом можно.
- [ ] G выкидывает из обеих мини-игр и НЕ бросает предмет в этот момент.
...
## Очередь модулей (после «Труба + Генератор»)

1. **Модели и вид**: импорт meshy-моделей на все заглушки (когда пользователь сгенерит), материалы.
2. **Анимации** (пользователь даёт файлы — подключаем AnimBP headless).
3. **Монстр-слухач** — ТОЛЬКО по отмашке.
