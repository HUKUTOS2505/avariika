...
void UGridDungeonQuery::GetCellDimension(int32 CellId, FVector& OutCenter, FVector& OutSize) {
    if (Model && Config) {
        ...
        if (Cell.CellType == EGridDungeonCellType::Unknown) {
            ...
            FVector LocationWorld = FDAMathUtils::ToVector(Cell.Bounds.Location) * GridSize;
            FVector SizeWorld = FDAMathUtils::ToVector(Cell.Bounds.Size) * GridSize;
...
FVector GetCellCenter(const FGridDungeonCell& Cell, const FVector& GridSize) {
    return (FDAMathUtils::ToVector(Cell.Bounds.Location) + FDAMathUtils::ToVector(Cell.Bounds.Size) / 2.0f) * GridSize;
}
...
FVector UGridDungeonQuery::GetOpeningPointBetweenAdjacentCells(int32 CellAId, int32 CellBId) {
    ...
    if (CellA.CellType == EGridDungeonCellType::Room || CellB.CellType == EGridDungeonCellType::Room) {
        if (ContainsDoorBetween(CellAId, CellBId)) {
            ...
            FVector LocA = FDAMathUtils::ToVector(Door.AdjacentTiles[0]) + FVector(0.5f, 0.5f, 0);
            FVector LocB = FDAMathUtils::ToVector(Door.AdjacentTiles[1]) + FVector(0.5f, 0.5f, 0);
    ...
    FVector InterLocation = FDAMathUtils::ToVector(Intersection.Location);
    FVector InterSize = FDAMathUtils::ToVector(Intersection.Size);
...
void UGridDungeonQuery::IsNearMarker(const FTransform& CurrentMarkerTransform, const FString& NearbyMarkerName,
                                     float NearbyDistance, UDungeonModel* DungeonModel, bool& bIsNear, int32& NumFound) {
    NumFound = 0;
    if (DungeonModel) {
        ...
        for (const FDungeonMarkerInstance& Marker : DungeonModel->WorldMarkers) {
            float DistanceSq = (Marker.Transform.GetLocation() - CurrentLocation).SizeSquared();
            if (DistanceSq < QueryDistanceSq && Marker.MarkerName == NearbyMarkerName) {
                NumFound++;
            }
        }
    }
