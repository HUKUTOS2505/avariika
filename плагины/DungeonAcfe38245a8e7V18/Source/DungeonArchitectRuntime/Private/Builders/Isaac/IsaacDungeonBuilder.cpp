...
void UIsaacDungeonBuilder::InitializeState() {
...
public:
    FDAIsaacRoomFactory(int InRoomWidth, int InRoomHeight) : roomWidth(InRoomWidth), roomHeight(InRoomHeight) {
    }
...
typedef TSharedPtr<FDAIsaacRoomFactory> FDAIsaacRoomFactoryPtr;


void UIsaacDungeonBuilder::GenerateLevelLayout() {
    auto startPosition = FIntVector::ZeroValue;
    TQueue<FDAIsaacLevelGrowthNode> queue;
    TSet<FIntVector> visited;

    FDAIsaacRoomFactoryPtr roomFactory = MakeShareable(
        new FDAIsaacRoomFactory(isaacConfig->RoomWidth, isaacConfig->RoomLength));
    rooms.Reset();
    doors.Reset();
...
void UIsaacDungeonBuilder::AddNextRoomNode(TSharedPtr<FDAIsaacRoomFactory> roomFactory, TQueue<FDAIsaacLevelGrowthNode>& queue,
                                           TSet<FIntVector>& visited,
    ...
    if (!visited.Contains(nextPosition)) {
        ...
        rooms.Add(nextRoom);
        FDAIsaacLevelGrowthNode nextNode;
...
void UIsaacDungeonBuilder::EmitDungeonMarkers_Implementation() {
    ...
    for (const FIsaacRoom& room : isaacModel->Rooms) {
        FVector roomBasePosition = FDAMathUtils::ToVector(room.Location) * (roomSizeWorld + roomPadding);
        ...
        for (int x = -1; x < roomWidth + 1; x++) {
            for (int y = -1; y < roomHeight + 1; y++) {
                ...
                if (tile.tileType == EIsaacRoomTileType::Floor) {
                    CreateMarkerAt(x + 0.5f, y + 0.5f, 0, FIsaacBuilderMarkers::GROUND, roomBasePosition, tileSize);
                }

            }
        }

        EmitBoundaryMarkers(FIsaacBuilderMarkers::WALL, EIsaacRoomTileType::Empty, EIsaacRoomTileType::Floor, room,
                            roomWidth, roomHeight, roomBasePosition, tileSize);
        EmitBoundaryMarkers(FIsaacBuilderMarkers::DOOR, EIsaacRoomTileType::Door, EIsaacRoomTileType::Floor, room,
                            roomWidth, roomHeight, roomBasePosition, tileSize);
    }
}
...
void UIsaacDungeonBuilder::GetDefaultMarkerNames(TArray<FString>& OutMarkerNames) {
    ...
    OutMarkerNames.Add(FIsaacBuilderMarkers::GROUND);
    OutMarkerNames.Add(FIsaacBuilderMarkers::WALL);
    OutMarkerNames.Add(FIsaacBuilderMarkers::DOOR);
}
