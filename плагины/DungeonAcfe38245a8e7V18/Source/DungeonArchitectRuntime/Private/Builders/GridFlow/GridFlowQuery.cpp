...
void UGridFlowQuery::GetFreeTileLocation(TArray<EGridFlowAbstractNodeRoomType> AllowedRoomTypes,
    ...
    const FVector TargetWorldCoord = ConvertTileToWorldCoord(FDAMathUtils::ToVector(TargetTileCoord));
...
void UGridFlowQuery::IsNearMarker(const FTransform& CurrentMarkerTransform, const FString& NearbyMarkerName,
                                  float NearbyDistance, UDungeonModel* Model, bool& bIsNear, int32& NumFound) {
    NumFound = 0;
    if (Model) {
        ...
        for (const FDungeonMarkerInstance& Marker : Model->WorldMarkers) {
            const float DistanceSq = (Marker.Transform.GetLocation() - CurrentLocation).SizeSquared();
