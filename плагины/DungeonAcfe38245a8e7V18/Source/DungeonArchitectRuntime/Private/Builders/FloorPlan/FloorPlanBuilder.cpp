...
void UFloorPlanBuilder::InitializeState() {
...
FRectangle InverseTransformBounds(ADungeon* Dungeon, const FVector& GridSize, const FRectangle& Bounds) {
    ...
    FVector Location = FDAMathUtils::ToVector(Bounds.Location) * GridSize;
    Location = DungeonInverseTransform.TransformPosition(Location);
    Location /= GridSize;
    FVector Size = DungeonInverseTransform.TransformVector(FDAMathUtils::ToVector(Bounds.Size));

    FRectangle FixedBounds;
    FixedBounds.Location = FDAMathUtils::ToIntVector(Location, true);
    FixedBounds.Size = FDAMathUtils::ToIntVector(Size, true);
    return FixedBounds;
}
...
void UFloorPlanBuilder::BuildLayout() {
    ...
    auto TagFloorPlanActors = [](UWorld* World) {
        if (World) {
            for (TActorIterator<AFloorPlanRoomVolume> It(World); It; ++It) {
                AFloorPlanRoomVolume* Actor = *It;
                if (Actor->Tags.Num() == 0) {
    ...
    for (int z = 0; z < NumFloors; z++) {
        ...
        TArray<FloorChunkPtr> FloorChunks;
        {
            ...
            InitialFloor->Bounds.Size = FDAMathUtils::ToIntVector(FloorPlanConfig->BuildingSize, true);
        ...
        if (World) {
            ...
            BuildingBounds.Size = FDAMathUtils::ToIntVector(FloorPlanConfig->BuildingSize, true);
...
FString GetDoorMarkerName(FloorChunkPtr ChunkA, FloorChunkPtr ChunkB) {
    if (!ChunkA.IsValid() && !ChunkB.IsValid()) {
        return FFloorPlanBuilderMarkers::DOOR;
    }
    ...
    return PreferedChunk->DoorMarker.Len() > 0 ? PreferedChunk->DoorMarker : FFloorPlanBuilderMarkers::DOOR;
}
...
void UFloorPlanBuilder::EmitDungeonMarkers_Implementation() {
    ...
    for (int z = 0; z < NumFloors; z++) {
        for (int x = -1; x < FloorPlanConfig->BuildingSize.X; x++) {
            ...
            for (int y = -1; y < FloorPlanConfig->BuildingSize.Y; y++) {
                ...
                FString GroundMarkerName = FFloorPlanBuilderMarkers::GROUND;
                FString CeilingMarkerName = FFloorPlanBuilderMarkers::CEILING;
                ...
                */
                ...
                FString DoorMarkerName = FFloorPlanBuilderMarkers::DOOR;
                ...
                if (bEmitLeftWall) {
                    ...
                    if (bLeftDoor) {
                        ...
                        MarkerName = FFloorPlanBuilderMarkers::WALL;
                        if (bBuildingWallLeft) {
                            MarkerName = FFloorPlanBuilderMarkers::BUILDING_WALL;
                        }
                        else {
    ...
    for (FloorChunkPtr Chunk : Chunks) {
        ...
        if (CenterMarker.Len() > 0 || PerFloorCenterMarker.Len() > 0) {
            FVector ChunkSize = FDAMathUtils::ToVector(Chunk->Bounds.Size) / 2.0f;
            ChunkSize.Z = 0;
            FVector GridLocation = FDAMathUtils::ToVector(Chunk->Bounds.Location) + ChunkSize;
            FVector WorldLocation = GridLocation * GridSize;
    ...
    EmitSeparatorMarkers(FFloorPlanBuilderMarkers::WALL, FFloorPlanBuilderMarkers::WALL_SEPARATOR, GridSize);
    EmitSeparatorMarkers(FFloorPlanBuilderMarkers::BUILDING_WALL, FFloorPlanBuilderMarkers::BUILDING_WALL_SEPARATOR, GridSize);
}
...
void UFloorPlanBuilder::EmitMarkerAt(const FVector& WorldLocation, const FString& MarkerName, const FQuat& Rotation) {
    ...
    AddMarker(MarkerName, Transform);
}
...
void UFloorPlanBuilder::EmitSeparatorMarkers(const FString& WallMarkerName, const FString& SepratorMarkerName,
    ...
    for (const FTransform& WallTransform : WallTransforms) {
        ...
        AddMarker(SepratorMarkerName, SepratorTransform);
    }
}
...
void DrawChunk(FloorChunkPtr Chunk, const FVector& GridSize, UWorld* InWorld, bool bPersistant, float lifeTime) {
    ...
    FVector Location = FDAMathUtils::ToVector(Chunk->Bounds.Location) * GridSize;
    FVector Size = FDAMathUtils::ToVector(Chunk->Bounds.Size) * GridSize;
...
void UFloorPlanBuilder::GetDefaultMarkerNames(TArray<FString>& OutMarkerNames) {
    ...
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::GROUND);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::CEILING);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::WALL);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::WALL_SEPARATOR);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::DOOR);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::BUILDING_WALL);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::BUILDING_WALL_SEPARATOR);
}

void UFloorPlanBuilder::CreateMarkerVisualizationRules(FDungeonMarkerVisualizationRules& OutRules) {
    ...
    OutRules.SetGridSize(FloorPlanConfig ? FloorPlanConfig->GridSize : FVector::ZeroVector);
    
    OutRules.Set(FFloorPlanBuilderMarkers::GROUND, { EDungeonMarkerVisualizationShape::Tile });
    OutRules.Set(FFloorPlanBuilderMarkers::CEILING, { EDungeonMarkerVisualizationShape::Tile });
    
    OutRules.Set(FFloorPlanBuilderMarkers::WALL, { EDungeonMarkerVisualizationShape::Wall });
    OutRules.Set(FFloorPlanBuilderMarkers::DOOR, { EDungeonMarkerVisualizationShape::Wall });
    OutRules.Set(FFloorPlanBuilderMarkers::BUILDING_WALL, { EDungeonMarkerVisualizationShape::Wall });
    
    OutRules.Set(FFloorPlanBuilderMarkers::WALL_SEPARATOR, { EDungeonMarkerVisualizationShape::Pillar });
    OutRules.Set(FFloorPlanBuilderMarkers::BUILDING_WALL_SEPARATOR, { EDungeonMarkerVisualizationShape::Pillar });
}

bool UFloorPlanBuilder::PerformSelectionLogic(const TArray<UDungeonSelectorLogic*>& SelectionLogics, const FDungeonMarkerInstance& InMarker) {
...
TSharedPtr<IMarkerGenProcessor> UFloorPlanBuilder::CreateMarkerGenProcessor(const FTransform& InDungeonTransform) const {
    if (const UFloorPlanConfig* Config = FloorPlanConfig ? FloorPlanConfig.Get() : Cast<UFloorPlanConfig>(DungeonConfig)) {
