...
void ADungeonInstancedMeshActor::OnBuildStop() {
    PurgeUnusedInstances();
    InstancedComponentCache.Reset();

    MarkComponentsRenderStateDirty();
}

void ADungeonInstancedMeshActor::PurgeUnusedInstances() {
...
UHierarchicalInstancedStaticMeshComponent* ADungeonInstancedMeshActor::GetInstancedComponent(UDungeonMesh* Mesh) {
    ...
    for (const FMaterialOverride& Override : Mesh->MaterialOverrides) {
        Hash = HashCombine(Hash, Override.index);
        Hash = HashCombine(Hash, GetTypeHash(Override.Material));
    }
    ...
    {
        if (TWeakObjectPtr<UHierarchicalInstancedStaticMeshComponent>* SearchResult = InstancedComponentCache.Find(Hash)) {
            TWeakObjectPtr<UHierarchicalInstancedStaticMeshComponent> WeakPtr = *SearchResult;
            if (UHierarchicalInstancedStaticMeshComponent* Component = WeakPtr.Get()) {
                return Component;
            }
            else {
                InstancedComponentCache.Remove(Hash);
            }
        }
    }
    ...
    Component->SetCanEverAffectNavigation(Mesh->bCanEverAffectNavigation);
