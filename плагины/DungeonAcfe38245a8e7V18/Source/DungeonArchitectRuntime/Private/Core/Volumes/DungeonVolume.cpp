...
void ADungeonVolume::GetDungeonVolumeBounds(const FVector& GridCellSize, FRectangle& OutBounds) const {
    const FBox Box = CalcLocalVolumeBounds();

    FVector BoxOrigin, BoxExtent;
    BoxOrigin = Box.GetCenter();
...
FBox ADungeonVolume::CalcLocalVolumeBounds() const {
    FTransform DungeonTransform = IsValid(Dungeon) ? Dungeon->GetActorTransform() : FTransform::Identity;
    FTransform InverseDungeonTransform = DungeonTransform.Inverse();
    FTransform VolumeTransform = GetActorTransform();
    FBox Box(ForceInit);
    {
        if (UBrushComponent* BrushComp = GetBrushComponent()) {
            if (BrushComp->Brush) {
                for (const FVert& Vert : BrushComp->Brush->Verts) {
                    if (BrushComp->Brush->Points.IsValidIndex(Vert.pVertex)) {
                        const FVector3f& Location3f = BrushComp->Brush->Points[Vert.pVertex];
                        FVector LocalPosition = InverseDungeonTransform.TransformPosition(VolumeTransform.TransformPosition(FVector(Location3f)));
                        Box += LocalPosition;
                    }
                }
            }
