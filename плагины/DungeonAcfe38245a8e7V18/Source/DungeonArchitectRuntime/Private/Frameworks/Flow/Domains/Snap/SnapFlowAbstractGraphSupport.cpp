				...
				TArray<FFlowAGPathNodeGroup>& OutGroups) const
{
	...
	for (const FSnapFlowAGNodeGroupSetting& GroupSetting : FilteredGroupSettings) {
		for (const FIntVector& LocalSurfaceCoord : GroupSetting.LocalSurfaceCoords) {
			bool bValid = true;
			FIntVector BaseCoord = FDAMathUtils::ToIntVector(Node->Coord, true) - LocalSurfaceCoord;
...
bool FSnapGridFlowAbstractGraphConstraints::IsValid(const FFlowAbstractGraphQuery& InGraphQuery, const FFlowAGPathNodeGroup& Group,
	...
	auto CanFit = [](const FSGFModuleAssembly& RegisteredAssembly, const FSGFModuleAssembly& AssemblyToFit, const TArray<FSnapGridFlowModuleDatabaseConnectionInfo>& ConnectionInfos) -> bool {
		TArray<FSGFModuleAssemblySideCell> DoorIndices;
		if (RegisteredAssembly.CanFit(AssemblyToFit, DoorIndices)) {
			for (const FSGFModuleAssemblySideCell& DoorCellInfo : DoorIndices) {
				check(ConnectionInfos.IsValidIndex(DoorCellInfo.ConnectionIdx));
				const FSnapGridFlowModuleDatabaseConnectionInfo& ConnectionInfo = ConnectionInfos[DoorCellInfo.ConnectionIdx];
				if (ConnectionInfo.ConnectionConstraint == ESnapConnectionConstraint::RoomEntry && DoorCellInfo.Direction != ESGFModuleAssemblySideCellDirection::Incoming) {
					// This should be a incoming link, since we enter the door from here
					return false;
				}
				if (ConnectionInfo.ConnectionConstraint == ESnapConnectionConstraint::RoomExit && DoorCellInfo.Direction != ESGFModuleAssemblySideCellDirection::Outgoing) {
					// This should be an outgoing link, since we exit the door from here
					return false;
				}
			}
			return true;
			...
			for (const FSnapGridFlowModuleDatabaseItem& Module : ModuleDatabase->Modules) {
				if (InAllowedCategories.Contains(Module.Category)) {
					...
					for (int StepIdx = 0; StepIdx < NumRotatedAssemblies; StepIdx++) {
						...
						TArray<FSGFModuleAssemblySideCell> DoorIndices;
						if (CanFit(RegisteredAssembly, Assembly, Module.Connections)) {
							return true;
						}
					}
				}
...
bool FSnapGridFlowAbstractGraphConstraints::IsValid(const FFlowAbstractGraphQuery& InGraphQuery, const UFlowAbstractNode* Node,
                                                    const TArray<const UFlowAbstractNode*>& InIncomingNodes, const TArray<const UFlowAbstractNode*>& InOutgoingNodes) {
	...
	TArray<const UFlowAbstractNode*> IncomingNodes = InIncomingNodes;
	TArray<const UFlowAbstractNode*> OutgoingNodes = InOutgoingNodes;
		...
		for (const UFlowAbstractLink* ConnectedLink : ConnectedLinks) {
			...
			if (ConnectedLink->Source == Node->NodeId || ConnectedLink->SourceSubNode == Node->NodeId) {
				if (const UFlowAbstractNode* ConnectedNode = FFlowAbstractGraphPathUtils::TryGetSubNodeFromLink(InGraphQuery, ConnectedLink->Destination, ConnectedLink)) {
					OutgoingNodes.AddUnique(ConnectedNode);
				}
			}
				...
				if (const UFlowAbstractNode* ConnectedNode = FFlowAbstractGraphPathUtils::TryGetSubNodeFromLink(InGraphQuery, ConnectedLink->Source, ConnectedLink)) {
					IncomingNodes.AddUnique(ConnectedNode);
				}
			}
		} 
	}

	for (const UFlowAbstractNode* ConnectedNode : IncomingNodes) {
		check(ConnectedNode->MergedCompositeNodes.Num() == 0)
	}
	for (const UFlowAbstractNode* ConnectedNode : OutgoingNodes) {
		check(ConnectedNode->MergedCompositeNodes.Num() == 0)
	}
	...
	FSGFNodeGroupGenLib::BuildNodeGroup(InGraphQuery, Node, IncomingNodes, OutgoingNodes, Group, ConstraintLinks);
...
void FSGFNodeGroupGenLib::BuildNodeGroup(const FFlowAbstractGraphQuery& InGraphQuery,
	                   const UFlowAbstractNode* InNode, const TArray<const UFlowAbstractNode*>& InIncomingNodes, const TArray<const UFlowAbstractNode*>& InOutgoingNodes,
	...
	if (InNode->MergedCompositeNodes.Num() <= 1) {
		...
		MinCoord = MaxCoord = FDAMathUtils::ToIntVector(InNode->Coord, true);
	}
	else {
		...
		MinCoord = FDAMathUtils::ToIntVector(MinCoordF, true);
		MaxCoord = FDAMathUtils::ToIntVector(MaxCoordF, true);

		for (UFlowAbstractNode* const SubNode : InNode->MergedCompositeNodes) {
			const FIntVector Coord = FDAMathUtils::ToIntVector(SubNode->Coord, true);
		...
		if (bHostsSource) {
			if (OutGroup.GroupEdgeNodes.Contains(Source)) {
				...
				if (SourceNode && DestinationNode) {
					OutConstraintLinks.Add(FFAGConstraintsLink(SourceNode, DestinationNode, EFAGConstraintsLinkDirection::Outgoing));
				}
			}
		}
		else if (bHostsDest) {
			if (OutGroup.GroupEdgeNodes.Contains(Destination)) {
				...
				if (!DestinationNode) DestinationNode = InGraphQuery.GetSubNode(Destination);
				if (SourceNode && DestinationNode) {
					OutConstraintLinks.Add(FFAGConstraintsLink(DestinationNode, SourceNode, EFAGConstraintsLinkDirection::Incoming));
				}
			}
		}
	}
	...
	for (UFlowAbstractNode* GraphNode : Graph->GraphNodes) {
		if (GraphNode->MergedCompositeNodes.Num() > 0) {
			for (UFlowAbstractNode* SubNode : GraphNode->MergedCompositeNodes) {
				FIntVector Coord = FDAMathUtils::ToIntVector(SubNode->Coord, true);
				UFlowAbstractNode*& NodeRef = NodeByCoords.FindOrAdd(Coord);
				NodeRef = SubNode;
			}
		}
		else {
			FIntVector Coord = FDAMathUtils::ToIntVector(GraphNode->Coord, true);
			UFlowAbstractNode*& NodeRef = NodeByCoords.FindOrAdd(Coord);
			NodeRef = GraphNode;
		}
	}
	...
	auto PopulateConstraints = [&](const TArray<const UFlowAbstractNode*>& InNodes, EFAGConstraintsLinkDirection Direction) {
		for (const UFlowAbstractNode* const IncomingNode : InNodes) {
			if (!IncomingNode) continue;
			FIntVector InnerCoord = FDAMathUtils::ToIntVector(IncomingNode->Coord, true);
			...
			if (InnerNodePtr) {
				...
				OutConstraintLinks.Add(FFAGConstraintsLink(InnerNode, IncomingNode, Direction));	
			}
		}
	};

	PopulateConstraints(InIncomingNodes, EFAGConstraintsLinkDirection::Incoming);
	PopulateConstraints(InOutgoingNodes, EFAGConstraintsLinkDirection::Outgoing);
}
