...
UFlowAbstractNode* UFlowAbstractGraphBase::CreateNode(const FRandomStream& InRandom, TSubclassOf<UFlowAbstractNode> NodeClassOverride) {
    TSubclassOf<UFlowAbstractNode> NodeClass = NodeClassOverride ? NodeClassOverride : GetNodeClass();
    UFlowAbstractNode* Node = NewObject<UFlowAbstractNode>(this, NodeClass);
...
namespace {
    template <typename T>
    void DACloneUObjectArray(UObject* Outer, const TArray<TObjectPtr<T>>& SourceList, TArray<TObjectPtr<T>>& DestList) {
        DestList.Reset();
        for (TObjectPtr<T> Source : SourceList) {
            if (!Source) continue;
...
TSet<FName> UFlowAbstractNode::GetHierarchyTags() const {
    TSet<FName> AllTags = Tags;
    for (TObjectPtr<UFlowAbstractNode> MergedNode : MergedCompositeNodes) {
        if (MergedNode) {
            AllTags.Append(MergedNode->Tags);
        }
    }
    return AllTags;
}
...
void UFlowAbstractLink::ReverseDirection() {
    FDAMathUtils::Swap(Source, Destination);
    FDAMathUtils::Swap(SourceSubNode, DestinationSubNode);
}
