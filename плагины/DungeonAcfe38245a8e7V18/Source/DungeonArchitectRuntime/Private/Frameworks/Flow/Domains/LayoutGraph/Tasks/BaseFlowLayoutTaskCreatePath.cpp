...
void UBaseFlowLayoutTaskCreatePath::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
	...
	FFlowAGPathingSystem PathingSystem(MaxFramesToProcess);
	{
		...
		TArray<int32> StartNodeIndices = FDAMathUtils::GetShuffledIndices(PossibleStartNodes.Num(), Random);
	...
	if (PathingSystem.FoundResult()) {
		...
		ExtendPathNodes(Output.State, PathResult.State, Random, GraphQuery);
