...
void UBaseFlowLayoutTaskCreateMainPath::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
    ...
    FFlowAGStaticGrowthState StaticState;
    {
        ...
        StaticState.NodeCreationConstraint = GetNodeCreationConstraintLogic(Input.Dungeon.Get());
    ...
    if (PathingSystem.FoundResult()) {
		...
		ExtendPathNodes(Output.State, PathResult.State, *Input.Random, GraphQuery);
...
TArray<int32> UBaseFlowLayoutTaskCreateMainPath::GetPossibleEntranceIndices(UFlowAbstractGraphBase* InGraph, const FRandomStream& InRandom) const {
    const TArray<int32> AllNodeIndices = FDAMathUtils::GetShuffledIndices(InGraph->GraphNodes.Num(), InRandom);
