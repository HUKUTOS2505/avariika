        ...
        : Super(ObjectInitializer)
{
    ...
    if (!IsRunningCommandlet()) {
        const TCHAR* PathName = TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/S_SnapDoor");
        ...
        struct FConstructorStatics {
            ...
            FConstructorStatics()
                : IconDoor(TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/S_SnapDoor"))
                , IconMagnet(TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/icon_Magnet_256x"))
                , IconPlugMale(TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/icon_PlugMale_256x"))
                , IconPlugFemale(TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/icon_PlugFemale_256x"))
                , IconRoomEntry(TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/icon_ConnRoomEntry"))
                , IconRoomExit(TEXT("/DungeonArchitect/Core/Runtime/Assets/Textures/icon_ConnRoomExit"))
                , MaterialDebugDecal(TEXT("/DungeonArchitect/Core/Runtime/Assets/Materials/Snap/Decals/M_SnapDecalBase")) {
            }
        };
...
void ASnapConnectionActor::BuildImpl(int32 InSeed, ADungeon* InDungeon, const FGuid& OwningModuleId, const FGuid& OtherModuleId, ULevel* HostLevel) {
    ...
    FDungeonMarkerInstance& StartMarker = MarkersToEmit.AddDefaulted_GetRef();
    StartMarker.Id = 0;
    ...
    TSharedPtr<FSnapThemeSceneProvider> SceneProvider = MakeShared<FSnapThemeSceneProvider>();
    ...
    ThemeEngineSettings.Dungeon = InDungeon;
    ThemeEngineSettings.LevelOverride = LevelOverride;

    const FRandomStream Random(InSeed);
    FDungeonThemeEngineEventHandlers ThemeEventHandlers;
    ThemeEventHandlers.PerformSelectionLogic = [this, InDungeon, &Random, OwningModuleId, OtherModuleId](const TArray<UDungeonSelectorLogic*>& SelectionLogics, const FDungeonMarkerInstance& InMarkerInfo) {
    ...
    ThemeEventHandlers.PerformGenericSelectionLogic = [this, InDungeon](const TArray<UGenericDungeonSelectorLogic*>& SelectionLogics, const FDungeonMarkerInstance& InMarkerInfo) {
        if (InDungeon && InDungeon->GetBuilder()) {
            return InDungeon->GetBuilder()->PerformGenericSelectionLogic(SelectionLogics, InMarkerInfo);
        }
        return false;
    };
