...
void FSnapGridFlowModuleResolver::GetCandidates(FSGFModuleResolveState& InResolveState, UFlowAbstractNode* InNode, int32 InDepth, const FSGFModuleAssembly& InAssembly, TArray<FModuleFitCandidate>& OutCandidates) const {
    ...
    TArray<int32> ModuleIndices = FDAMathUtils::GetShuffledIndices(ModuleItems.Num(), Random);
    for (int32 ModuleIdx : ModuleIndices) {
        ...
        TArray<int32> ShuffledAsmIndices = FDAMathUtils::GetShuffledIndices(NumAssemblies, Random);
        for (int32 AsmIdx : ShuffledAsmIndices) {
            ...
            if (ModuleAssembly.CanFit(InAssembly, DoorIndices)) {
                ...
                for (const FSGFModuleAssemblySideCell& DoorInfo : DoorIndices) {
                    check(DoorInfo.Direction != ESGFModuleAssemblySideCellDirection::Unknown);
                    ...
                    if (NodeConnection.ConnectionConstraint == ESnapConnectionConstraint::RoomEntry && DoorInfo.Direction != ESGFModuleAssemblySideCellDirection::Incoming) {
                        // This should be a incoming link, since we enter the door from here
                        bDoorCategoriesValid = false;
                        break;
                    }
                    if (NodeConnection.ConnectionConstraint == ESnapConnectionConstraint::RoomExit && DoorInfo.Direction != ESGFModuleAssemblySideCellDirection::Outgoing) {
...
void FSnapGridFlowModuleResolver::RegisterNodeModule(UFlowAbstractNode* InNode, FSGFModuleResolveState& InResolveState, const FModuleFitCandidate& InCandidate) const {
    ...
    FIntVector Coord = FDAMathUtils::ToIntVector(InNode->Coord, true);
    for (UFlowAbstractNode* SubNode : InNode->MergedCompositeNodes) {
        if (!SubNode) continue;
        const FIntVector SubCoord = FDAMathUtils::ToIntVector(SubNode->Coord, true);
