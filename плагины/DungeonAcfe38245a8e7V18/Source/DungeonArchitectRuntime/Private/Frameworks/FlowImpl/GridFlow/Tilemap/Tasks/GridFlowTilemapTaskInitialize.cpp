...
UGridFlowTilemap* UGridFlowTilemapTaskInitialize::BuildTilemap(UGridFlowAbstractGraph* Graph, const FRandomStream& Random) const {
    ...
    for (const UFlowAbstractNode* Node : Graph->GraphNodes) {
        FIntVector Coord = FDAMathUtils::ToIntVector(Node->Coord, true);
...
void UGridFlowTilemapTaskInitialize::FixCorridorSizes(FGridFlowTilemapNodes& TileNodes, const FFlowAbstractGraphQuery& GraphQuery) const {
    ...
    for (FGridFlowTilemapNodeInfo& TileNode : TileNodes.GetCells()) {
        ...
        if (Node && Node->FindOrAddDomainData<UFANodeTilemapDomainData>()->RoomType == EGridFlowAbstractNodeRoomType::Corridor) {
            ...
            const FIntVector InCoord = FDAMathUtils::ToIntVector(IncomingNode->Coord, true);
            const FIntVector OutCoord = FDAMathUtils::ToIntVector(OutgoingNode->Coord, true);
...
void UGridFlowTilemapTaskInitialize::GenerateCaveMap(FGFCaveCellBuildTiles& CaveMap, FGridFlowTilemapNodes& TileNodes,
    ...
    for (const FFlowTilemapCell& Cell : Tilemap->GetCells()) {
        ...
        FGridFlowTilemapNodeInfo& TileNode = TileNodes[FDAMathUtils::ToIntVector(Cell.ChunkCoord, true)];
        UFlowAbstractNode* TileNodePtr = GraphQuery.GetNode(TileNode.AbstractNodeId);
        if (TileNodePtr) {
...
void UGridFlowTilemapTaskInitialize::BuildCaveStep_Cleanup(FGFCaveCellBuildTiles& CaveMap,
    ...
    for (int y = 0; y < Height; y++) {
        for (int x = 0; x < Width; x++) {
            ...
            FIntVector NodeCoord = FDAMathUtils::ToIntVector(Cell.ChunkCoord, true);
...
void UGridFlowTilemapTaskInitialize::BuildDoors(FGridFlowTilemapNodes& TileNodes, UGridFlowTilemap* Tilemap,
    ...
    for (FGridFlowTilemapNodeInfo& TileNode : TileNodes.GetCells()) {
        ...
        for (FGuid LinkId : Graph->GetOutgoingLinks(TileNodePtr->NodeId)) {
            ...
            if (DoorCell) {
                ...
                DoorMeta.ChunkA = FDAMathUtils::ToIntVector(SourceNode->Coord, true);
                DoorMeta.ChunkB = FDAMathUtils::ToIntVector(DestNode->Coord, true);
    ...
    for (const FDoorInfo& Door : DoorList) {
        ...
        TArray<TObjectPtr<UFlowGraphItem>>& Items = Link->LinkItems;
        TObjectPtr<UFlowGraphItem> DoorItem = Items.Num() > 0 ? Items[0] : nullptr;
...
bool UGridFlowTilemapTaskInitialize::ShouldBlockCaveBoundary(UGridFlowAbstractGraph* Graph, UFlowAbstractNode* CaveNode, int dx, int dy) const {
    const FIntVector Coord = FDAMathUtils::ToIntVector(CaveNode->Coord, true);
    ...
    for (UFlowAbstractNode* BaseNode : Graph->GraphNodes) {
        ...
        if (Node && FDAMathUtils::ToIntVector(Node->Coord, true) == OtherCoord) {
            OtherNode = Node;
            break;
        }
    }
...
void UGridFlowTilemapTaskInitialize::CalculateDistanceFromMainPath(FGridFlowTilemapNodes& TileNodes,
    ...
    while (Queue.Dequeue(Tile)) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nx >= 0 && nx < Width && ny >= 0 && ny < Height) {
                FFlowTilemapCell& NCell = Tilemap->Get(nx, ny);
                FGridFlowTilemapNodeInfo& NTileNode = TileNodes[FDAMathUtils::ToIntVector(NCell.ChunkCoord, true)];
