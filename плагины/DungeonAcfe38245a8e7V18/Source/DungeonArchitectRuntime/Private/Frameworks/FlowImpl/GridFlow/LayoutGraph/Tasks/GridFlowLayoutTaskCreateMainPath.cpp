...
TArray<int32> UGridFlowLayoutTaskCreateMainPath::GetPossibleEntranceIndices(UFlowAbstractGraphBase* InGraph, const FRandomStream& InRandom) const {
    ...
    if (bOverrideStartGridPosition) {
        ...
        for (int i = 0; i < InGraph->GraphNodes.Num(); i++) {
            ...
            if (Node && FDAMathUtils::ToIntPoint(FVector2D(Node->Coord), true) == StartGridPosition) {
...
void UGridFlowLayoutTaskCreateMainPath::FinalizePath(const FFlowAGStaticGrowthState& StaticState, FFlowAGGrowthState& GrowthState) const {
    Super::FinalizePath(StaticState, GrowthState);
    
    for (int i = 0; i < GrowthState.Path.Num(); i++) {
        const FFlowAGGrowthState_PathItem& PathItem = GrowthState.Path[i];
        ...
        else if (i == GrowthState.Path.Num() - 1) {
