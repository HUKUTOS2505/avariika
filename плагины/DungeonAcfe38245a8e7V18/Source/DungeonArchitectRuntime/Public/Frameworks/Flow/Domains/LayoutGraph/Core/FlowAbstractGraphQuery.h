...
public:
    ...
    TNode* GetNodeObjectAtCoord(const FIntVector& InCoord) const {
        return GetNodeObjectAtCoord(FDAMathUtils::ToVector(InCoord));
    }
    ...
    UFlowAbstractLink* GetLink(const FGuid& LinkId) const {
        const TWeakObjectPtr<UFlowAbstractLink>* SearchResult = LinkMap.Find(LinkId);
        if (!SearchResult) return nullptr;
