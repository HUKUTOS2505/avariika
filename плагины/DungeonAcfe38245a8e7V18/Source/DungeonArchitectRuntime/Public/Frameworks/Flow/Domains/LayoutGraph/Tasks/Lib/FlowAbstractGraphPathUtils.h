...
class DUNGEONARCHITECTRUNTIME_API UFlowLayoutNodeCreationConstraint : public UObject {
    GENERATED_BODY()
public:
    virtual bool CanCreateNodeAt(const UFlowAbstractNode* Node, int32 TotalPathLength, int32 CurrentPathPosition);
...
public:
    ...
    UPROPERTY()
    FName StartNodePathName;
    
    UPROPERTY()
    FName EndNodePathName;

    UPROPERTY()
    TArray<TWeakObjectPtr<ADungeonFlowTagVolume>> ConstraintVolumes;
    
    UPROPERTY()
    FTransform CoordToWorld = FTransform::Identity;
};
