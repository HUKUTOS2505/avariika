...
#include "AbilitySystem/Executions/DAGDamageExecution.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DAGDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UDAG_CombatAttributeSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UDAG_DamageExecution::UDAG_DamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UDAG_DamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FDAG_GameplayEffectContext* TypedContext = FDAG_GameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UDAG_TeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UDAG_TeamSubsystem>();
	...
	if (const IDAG_AbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UDAG_HealthAttributeSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
