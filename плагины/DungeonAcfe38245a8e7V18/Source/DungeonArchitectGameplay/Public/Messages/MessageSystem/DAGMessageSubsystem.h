...
#include "DAGMessageSubsystem.generated.h"

class UDAG_MessageSubsystem;
struct FFrame;

DUNGEONARCHITECTGAMEPLAY_API DECLARE_LOG_CATEGORY_EXTERN(LogDAGMessageSubsystem, Log, All);

class UAsyncAction_ListenForDAGMessage;
...
struct DUNGEONARCHITECTGAMEPLAY_API FDAG_MessageListenerHandle
{
public:
	GENERATED_BODY()

	FDAG_MessageListenerHandle() {}
...
private:
	UPROPERTY(Transient)
	TWeakObjectPtr<UDAG_MessageSubsystem> Subsystem;
	...
	friend UDAG_MessageSubsystem;

	FDAG_MessageListenerHandle(UDAG_MessageSubsystem* InSubsystem, FGameplayTag InChannel, int32 InID) : Subsystem(InSubsystem), Channel(InChannel), ID(InID) {}
};
 ...
 */
USTRUCT()
struct FDAG_MessageListenerData
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class DUNGEONARCHITECTGAMEPLAY_API UDAG_MessageSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()

	friend UAsyncAction_ListenForDAGMessage;
...
public:
	 ...
	 */
	static UDAG_MessageSubsystem& Get(const UObject* WorldContextObject);
	 ...
	 */
	template <typename FMessageStructType>
	FDAG_MessageListenerHandle RegisterListener(FGameplayTag Channel, TFunction<void(FGameplayTag, const FMessageStructType&)>&& Callback, EDAG_MessageMatch MatchType = EDAG_MessageMatch::ExactMatch)
	{
	...
	FDAG_MessageListenerHandle RegisterListener(FGameplayTag Channel, TOwner* Object, void(TOwner::* Function)(FGameplayTag, const FMessageStructType&))
	{
	 ...
	 */
	template <typename FMessageStructType>
	FDAG_MessageListenerHandle RegisterListener(FGameplayTag Channel, FDAG_MessageListenerParams<FMessageStructType>& Params)
	{
		FDAG_MessageListenerHandle Handle;
	 ...
	 */
	void UnregisterListener(FDAG_MessageListenerHandle Handle);
...
private:
	...
	FDAG_MessageListenerHandle RegisterListenerInternal(
		FGameplayTag Channel, 
...
private:
	...
	{
		TArray<FDAG_MessageListenerData> Listeners;
		int32 HandleID = 0;
	};
