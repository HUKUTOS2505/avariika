 ...
 * overlay. Just place the widget that you want to be draggable as the contents of SDADraggableBoxOverlay.
 */
class SDADraggableBoxOverlay : public SCompoundWidget
{
public:

	SLATE_BEGIN_ARGS(SDADraggableBoxOverlay) {}
...
/**
 * A drag/drop operation used by SDADraggableBox, largely modeled on FInViewportUIDragOperation, except
 ...
 */
class FDADraggableBoxUIDragOperation : public FDragDropOperation
{
public:

	DRAG_DROP_OPERATOR_TYPE(FDADraggableBoxUIDragOperation, FDragDropOperation)
	 ...
	 */
	static TSharedRef<FDADraggableBoxUIDragOperation> New(const TSharedRef<class SWidget>& InUIToBeDragged, const FVector2D InDecoratorOffset, 
		const FVector2D& OwnerAreaSize, SDADraggableBox::FOnDragComplete& OnDragComplete);

	virtual ~FDADraggableBoxUIDragOperation() {}

protected:
	/** The constructor is protected, so that this class can only be instantiated as a shared pointer. */
	FDADraggableBoxUIDragOperation(const TSharedRef<class SWidget>& InUIToBeDragged, const FVector2D InDecoratorOffsetFromCursor, 
		const FVector2D& OwnerAreaSize, SDADraggableBox::FOnDragComplete& OnDragCompletee);
	...
	SDADraggableBox::FOnDragComplete OnDragComplete;
};
