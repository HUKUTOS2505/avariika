...
#include "Frameworks/Algorithms/Layout/VerletGraphLayout.h"
...
void FVerletGraphLayout::PerformLayout(UEdGraph* Graph) {
    ...
    // Create the verlet graph
    {
        ...
        for (UEdGraphNode* Node : Graph->Nodes) {
            UEdGraphNode* TaskNode = Cast<UEdGraphNode>(Node);
            if (TaskNode) {
        ...
        for (UEdGraphNode* Node : Graph->Nodes) {
            if (Node) {
                if (UEdGraphPin* OutputPin = FDungeonGraphUtils::GetFirstOutputPin(Node)) {
                    for (UEdGraphPin* OutgoingPin : OutputPin->LinkedTo) {
                        if (UEdGraphNode* OutgoingNode = OutgoingPin->GetOwningNode()) {
                            FLayoutVerletLinkPtr Constraint = MakeShareable(new FLayoutVerletLink);
                            Constraint->NodeA = NodeToVerletMap[Node];
                            Constraint->NodeB = NodeToVerletMap[OutgoingNode];
