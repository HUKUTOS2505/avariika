...
void SGrammarEditor::Construct(const FArguments& InArgs, TWeakObjectPtr<UGraphGrammar> InGrammar) {
    ...
    RuleListView = SNew(SEditableListView<TObjectPtr<UGraphGrammarProduction>>)
    ...
    NodeTypeListView = SNew(SEditableListView<TObjectPtr<UGrammarNodeType>>)
...
void SGrammarEditor::OnRuleSelectionChanged(TObjectPtr<UGraphGrammarProduction> Item, ESelectInfo::Type SelectInfo) {
...
FText SGrammarEditor::GetRuleListRowText(TObjectPtr<UGraphGrammarProduction> InItem) const {
    FText RuleName = InItem ? InItem->RuleName : LOCTEXT("MissingRuleName", "Unknown");
    return RuleName;
}

const TArray<TObjectPtr<UGraphGrammarProduction>>* SGrammarEditor::GetRuleList() const {
    return Grammar.IsValid() ? &Grammar->ProductionRules : nullptr;
}

void SGrammarEditor::OnRuleDelete(TObjectPtr<UGraphGrammarProduction> InItem) {
...
void SGrammarEditor::OnRuleReordered(TObjectPtr<UGraphGrammarProduction> Source, TObjectPtr<UGraphGrammarProduction> Dest) {
...
void SGrammarEditor::OnNodeTypeSelectionChanged(TObjectPtr<UGrammarNodeType> Item, ESelectInfo::Type SelectInfo) {
    if (PropertyEditor.IsValid()) {
        TSharedPtr<IDetailsView> PropertyEditorPtr = PropertyEditor.Pin();
        PropertyEditorPtr->SetObject(Item);
    }
...
FText SGrammarEditor::GetNodeTypeName(TObjectPtr<UGrammarNodeType> InItem) const {
    if (InItem) {
...
FText SGrammarEditor::GetNodeTypeDescription(TObjectPtr<UGrammarNodeType> InItem) const {
    if (InItem) {
...
const TArray<TObjectPtr<UGrammarNodeType>>* SGrammarEditor::GetNodeTypeList() {
    return Grammar.IsValid() ? &Grammar->NodeTypes : nullptr;
}

void SGrammarEditor::OnNodeTypeDelete(TObjectPtr<UGrammarNodeType> InItem) {
...
void SGrammarEditor::OnNodeTypeReordered(TObjectPtr<UGrammarNodeType> Source, TObjectPtr<UGrammarNodeType> Dest) {
    PerformReordering(Source, Dest, Grammar->NodeTypes);
    Grammar->Modify();
}

TSharedPtr<SWidget> SGrammarEditor::CreateNodeListItem(TObjectPtr<UGrammarNodeType> InItem) {
