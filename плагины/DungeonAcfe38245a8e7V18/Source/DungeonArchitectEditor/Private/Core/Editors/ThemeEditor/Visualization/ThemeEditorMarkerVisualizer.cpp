	...
	static void CreateBuilders(UDungeonConfig* InDungeonConfig, UDungeonModel* InDungeonModel, UDungeonBuilder* InDungeonBuilder, const TSet<FName>& MarkersToVisualize, TMap<FName, FDAMarkerVisMeshBuilderPtr>& OutMeshBuilders) {
		...
		if (InDungeonBuilder->IsA<UGridDungeonBuilder>()) {
			if (UGridDungeonConfig* GridConfig = Cast<UGridDungeonConfig>(InDungeonConfig)) {
				...
				DA_ADD_BUILDER(FGridBuilderMarkers::GROUND, FDAMarkerVisMeshBuilder_Ground)
                DA_ADD_BUILDER(FGridBuilderMarkers::WALL, FDAMarkerVisMeshBuilder_Wall)
                DA_ADD_BUILDER(FGridBuilderMarkers::FENCE, FDAMarkerVisMeshBuilder_Fence)
                DA_ADD_BUILDER(FGridBuilderMarkers::DOOR, FDAMarkerVisMeshBuilder_Door)
                DA_ADD_BUILDER(FGridBuilderMarkers::STAIR, FDAMarkerVisMeshBuilder_Stairs)
            }
		}
			...
			if (UFloorPlanConfig* FloorConfig = Cast<UFloorPlanConfig>(InDungeonConfig)) {
				...
				DA_ADD_BUILDER(FFloorPlanBuilderMarkers::GROUND, FDAMarkerVisMeshBuilder_Ground)
				DA_ADD_BUILDER(FFloorPlanBuilderMarkers::WALL, FDAMarkerVisMeshBuilder_Wall)
				DA_ADD_BUILDER(FFloorPlanBuilderMarkers::DOOR, FDAMarkerVisMeshBuilder_Door)
				DA_ADD_BUILDER(FFloorPlanBuilderMarkers::BUILDING_WALL, FDAMarkerVisMeshBuilder_Wall)
            }
		}
			...
			if (USimpleCityConfig* CityConfig = Cast<USimpleCityConfig>(InDungeonConfig)) {
				...
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::HOUSE, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::PARK, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::ROAD, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::ROAD_X, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::ROAD_T, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::ROAD_S, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::ROAD_CORNER, FDAMarkerVisMeshBuilder_TileEmpty)
				DA_ADD_BUILDER(FSimpleCityBuilderMarkers::ROAD_END, FDAMarkerVisMeshBuilder_TileEmpty)
            }
		}
			...
			if (Config && Model) {
				...
				DA_ADD_BUILDER(FGridFlowBuilderMarkers::GROUND, FDAMarkerVisMeshBuilder_Ground)
				DA_ADD_BUILDER(FGridFlowBuilderMarkers::CAVE_FENCE, FDAMarkerVisMeshBuilder_Fence)
                DA_ADD_BUILDER(FGridFlowBuilderMarkers::FENCE, FDAMarkerVisMeshBuilder_Fence)

				if (bWallsAsEdges) {
					DA_ADD_BUILDER(FGridFlowBuilderMarkers::WALL, FDAMarkerVisMeshBuilder_Wall)
					DA_ADD_BUILDER(FGridFlowBuilderMarkers::DOOR, FDAMarkerVisMeshBuilder_Door)
                    DA_ADD_BUILDER(FGridFlowBuilderMarkers::DOOR_ONEWAY, FDAMarkerVisMeshBuilder_Door)
                }
				else {
					DA_ADD_BUILDER(FGridFlowBuilderMarkers::WALL, FDAMarkerVisMeshBuilder_WallTile)
                    DA_ADD_BUILDER(FGridFlowBuilderMarkers::WALL_L, FDAMarkerVisMeshBuilder_WallTile)
                    DA_ADD_BUILDER(FGridFlowBuilderMarkers::WALL_T, FDAMarkerVisMeshBuilder_WallTile)
                    DA_ADD_BUILDER(FGridFlowBuilderMarkers::WALL_X, FDAMarkerVisMeshBuilder_WallTile)
                    DA_ADD_BUILDER(FGridFlowBuilderMarkers::DOOR, FDAMarkerVisMeshBuilder_DoorTile)
                    DA_ADD_BUILDER(FGridFlowBuilderMarkers::DOOR_ONEWAY, FDAMarkerVisMeshBuilder_DoorTile)
				}
            }
		}
	}
...
void AThemeEditorMarkerVisualizer::BuildVisualization(UDungeonConfig* InConfig, UDungeonModel* InModel, UDungeonBuilder* InBuilder, const TSet<FName>& MarkersToVisualize) const {
	...
	for (const FName& MarkerToVisualize : MarkersToVisualize) {
		...
		for (const FDungeonMarkerInstance& Marker : InModel->WorldMarkers) {
