...
void FMarkerGeneratorAppMode::Init(TSubclassOf<UDungeonBuilder> InBuilderClass) {
	...
	PatternList = SNew(SEditableListView<TObjectPtr<UMarkerGenLayer>>)
		...
		if (Obj->IsA<UMarkerGenPatternRule>()) {
			...
			if (SelectedPatternRule->RuleEdGraph && PatternRuleGraphEditor.IsValid()) {
				...
				const bool bNeedsReconstruction =
					...
					(Obj->IsA<UMGPatternScriptNode_RenameMarkers>() && PropertyName == GET_MEMBER_NAME_CHECKED(UMGPatternScriptNode_RenameMarkers, SourceMarkerName)) ||
					(Obj->IsA<UMGPatternScriptNode_RenameMarkers>() && PropertyName == GET_MEMBER_NAME_CHECKED(UMGPatternScriptNode_RenameMarkers, TargetMarkerName)) ||
					(Obj->IsA<UMGPatternScriptNode_CopyMarkers>() && PropertyName == GET_MEMBER_NAME_CHECKED(UMGPatternScriptNode_CopyMarkers, SourceMarkerName)) ||
					(Obj->IsA<UMGPatternScriptNode_CopyMarkers>() && PropertyName == GET_MEMBER_NAME_CHECKED(UMGPatternScriptNode_CopyMarkers, TargetMarkerName))
				;
...
void FMarkerGeneratorAppMode::OnLayerSelectionChanged(TObjectPtr<UMarkerGenLayer> Item, ESelectInfo::Type SelectInfo) {
	SetActiveLayer(Item);
}

TSharedPtr<SWidget> FMarkerGeneratorAppMode::CreateLayerListItemWidget(TObjectPtr<UMarkerGenLayer> InLayer) {
...
FText FMarkerGeneratorAppMode::GetLayerListRowText(TObjectPtr<UMarkerGenLayer> InItem) const {
	static const FText UnknownText = LOCTEXT("MissingLayerName", "Unknown");
	return InItem ? InItem->LayerName : UnknownText;
}

FText FMarkerGeneratorAppMode::GetLayerListRowFeatureText(TObjectPtr<UMarkerGenLayer> InItem) const {
...
const TArray<TObjectPtr<UMarkerGenLayer>>* FMarkerGeneratorAppMode::GetLayerList() const {
...
void FMarkerGeneratorAppMode::OnLayerDelete(TObjectPtr<UMarkerGenLayer> InItem) {
	if (!InItem) return;
...
void FMarkerGeneratorAppMode::OnLayerReordered(TObjectPtr<UMarkerGenLayer> Source, TObjectPtr<UMarkerGenLayer> Dest) const {
	if (UMarkerGenModel* Model = GetMarkerGenerationModel()) {
		int32 DestIndex = -1;
...
EVisibility FMarkerGeneratorAppMode::IsLayerListRowFeatureVisible(TObjectPtr<UMarkerGenLayer> InLayer) const {
...
FSlateColor FMarkerGeneratorAppMode::GetLayerListRowColor(TObjectPtr<UMarkerGenLayer> InLayer) const {
	if (!AppModeImpl.IsValid() || !AppModeImpl->IsLayerCompatible(InLayer)) {
...
void FMarkerGeneratorAppMode::OnRuleGraphUpdated() const {
	...
	if (PatternEditor.IsValid() && SelectedPatternRule.IsValid()) {
