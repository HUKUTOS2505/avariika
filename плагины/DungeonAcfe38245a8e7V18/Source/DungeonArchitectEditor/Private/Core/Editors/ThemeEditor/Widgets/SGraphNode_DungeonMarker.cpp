...
void SGraphNode_DungeonMarker::Construct(const FArguments& InArgs, UEdGraphNode_DungeonThemeMarker* InNode) {
    GraphNode = InNode;
...
bool SGraphNode_DungeonMarker::IsNodeUserDefined() const {
    UEdGraphNode_DungeonThemeMarker* Marker = Cast<UEdGraphNode_DungeonThemeMarker>(GraphNode);
    return Marker ? Marker->bUserDefined : false;
}

bool SGraphNode_DungeonMarker::IsNameReadOnly() const {
    return true;    // We want to disable editing the text with a click, since it interferes with the other theme edmode tools
    //return !IsNodeUserDefined();
}

void SGraphNode_DungeonMarker::OnPropertyChanged(UEdGraphNode_DungeonThemeBase* Sender, const FName& PropertyName) {
    UpdateGraphNode();
}

void SGraphNode_DungeonMarker::OnNameTextCommited(const FText& InText, ETextCommit::Type CommitInfo) {
    UEdGraphNode_DungeonThemeMarker* MarkerNode = Cast<UEdGraphNode_DungeonThemeMarker>(GraphNode);
    MarkerNode->MarkerName = InText.ToString();
    GraphNode->GetGraph()->NotifyGraphChanged();

    UEdGraph_DungeonTheme* DungeonGraph = Cast<UEdGraph_DungeonTheme>(GraphNode->GetGraph());
    if (DungeonGraph) {
        FEdGraphEditAction PropertyAction(GRAPHACTION_Default, DungeonGraph, MarkerNode, true);
        DungeonGraph->NotifyNodePropertyChanged(PropertyAction, FPropertyChangedEvent(nullptr));
    }
...
void SGraphNode_DungeonMarker::CreatePinWidgets() {
    UEdGraphNode_DungeonThemeMarker* DungeonNode = CastChecked<UEdGraphNode_DungeonThemeMarker>(GraphNode);
