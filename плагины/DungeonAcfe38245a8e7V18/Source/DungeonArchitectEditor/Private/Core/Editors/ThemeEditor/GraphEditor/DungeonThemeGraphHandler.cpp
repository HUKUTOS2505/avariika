...
void FDungeonArchitectThemeGraphHandler::Initialize(TSharedPtr<SGraphEditor> InGraphEditor, TWeakPtr<IDetailsView> InPropertyEditor, UEdGraph* InThemeGraph) {
...
void FDungeonArchitectThemeGraphHandler::PasteNodesHere(const FVector2f& Location) {
    TSharedPtr<SGraphEditor> GraphEditor = GetGraphEditor();
    ...
    for (TSet<UEdGraphNode*>::TIterator It(PastedNodes); It; ++It) {
        UEdGraphNode* Node = *It;
        if (UEdGraphNode_DungeonThemeMesh* MeshNode = Cast<UEdGraphNode_DungeonThemeMesh>(Node)) {
            // TODO: Handle
        }
        else if (UEdGraphNode_DungeonThemeActorTemplate* ActorTemplateNode = Cast<UEdGraphNode_DungeonThemeActorTemplate>(Node)) {
            // TODO: Handle
        }
        else if (UEdGraphNode_DungeonThemeMarker* MarkerNode = Cast<UEdGraphNode_DungeonThemeMarker>(Node)) {
...
void FDungeonArchitectThemeGraphHandler::ShowDefaultEditorProperties() const {
    if (FuncGetEditorProperties) {
        ShowProperties(FuncGetEditorProperties());
    }
    else {
