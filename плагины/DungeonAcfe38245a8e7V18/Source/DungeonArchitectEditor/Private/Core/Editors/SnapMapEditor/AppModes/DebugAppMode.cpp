...
void ASnapMapFlowEditorVisualization::UpdateConnectionState(const FGuid& InNodeId, const FGuid& InConnectionId, bool bIsDoor) {
    TObjectPtr<ULevelStreamingDynamic>* SearchResult = LoadedLevels.Find(InNodeId);
    if (SearchResult) {
    ...
    TObjectPtr<UPackage>* PackageSearchResult = LoadedPackages.Find(InNodeId);
    if (PackageSearchResult) {
        if (UPackage* Package = *PackageSearchResult) {
...
void ASnapMapFlowEditorVisualization::UnloadLevel(const FGuid& InNodeId) {
    UWorld* World = GetWorld();
    if (TObjectPtr<ULevelStreamingDynamic>* SearchResult = LoadedLevels.Find(InNodeId)) {
        if (ULevelStreamingDynamic* OldLevelStreaming = *SearchResult) {
    ...
    if (TObjectPtr<UPackage>* PackageSearchResult = LoadedPackages.Find(InNodeId)) {
...
void ASnapMapFlowEditorVisualization::UnloadAllLevels() {
    ...
    TArray<TObjectPtr<UPackage>> PackagesToUnload;
