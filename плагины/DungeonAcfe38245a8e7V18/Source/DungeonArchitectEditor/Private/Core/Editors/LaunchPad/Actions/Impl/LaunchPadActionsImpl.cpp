...
bool FLaunchPadActions::Exec_CloneScene(const FString& InObjectPath) {
    FString InPath = FPackageName::ObjectPathToPackageName(InObjectPath);
    ...
    if (ShowSaveDialog<UWorld>(InPath, SaveObjectPath)) {
        ...
        UWorld* ClonedAsset;
        if (CloneAsset(SourceAsset, SaveObjectPath, ClonedAsset, TargetPackageName)) {
            Exec_OpenFolder(TargetPackageName);
...
bool FLaunchPadActions::IsURLValid(const FString& URL)
{
    const TSet<FString> AllowedRootDomains = {
        TEXT("dungeonarchitect.dev"),
        TEXT("fab.com"),
        TEXT("unrealengine.com"),
        TEXT("epicgames.com"),
        TEXT("youtube.com"),
        TEXT("youtu.be"),      // Short form
        TEXT("discord.gg"),    // Invites
        TEXT("discord.com")    // Web interface
    };

    FString DomainToCheck = URL;

    // Strip Protocol (http:// or https://)
    if (DomainToCheck.StartsWith(TEXT("https://"))) {
        DomainToCheck.RemoveFromStart(TEXT("https://"));
    }
    else if (DomainToCheck.StartsWith(TEXT("http://"))) {
        DomainToCheck.RemoveFromStart(TEXT("http://"));
    }
    else {
        UE_LOG(LogTemp, Warning, TEXT("Blocked URL with invalid protocol: %s"), *URL);
        return false;
    }

    // Strip everything after the first slash (Path), Question mark (Query), or Hash
    int32 Index;
    if (DomainToCheck.FindChar('/', Index)) DomainToCheck = DomainToCheck.Left(Index);
    if (DomainToCheck.FindChar('?', Index)) DomainToCheck = DomainToCheck.Left(Index);
    if (DomainToCheck.FindChar('#', Index)) DomainToCheck = DomainToCheck.Left(Index);
    DomainToCheck = DomainToCheck.ToLower();

    bool bIsAllowed = false;
    for (const FString& AllowedDomain : AllowedRootDomains) {
        if (DomainToCheck == AllowedDomain)
        {
            bIsAllowed = true;
            break;
        }

        // Subdomain Match (e.g., "www.fab.com" ends with ".fab.com")
        // We prepend a dot to ensure we don't match "myfab.com" against "fab.com"
        if (DomainToCheck.EndsWith(TEXT(".") + AllowedDomain)) {
            bIsAllowed = true;
            break;
        }
    }
...
void FLaunchPadActions::Exec_OpenURL(const FString& URL) {
    // Allow only websites within the specific domains
    if (!IsURLValid(URL)) {
        UE_LOG(LogLaunchPadActionsImpl, Log, TEXT("Disallowed URL.  Blocked by DA: %s"), *URL);
        return;
    }
