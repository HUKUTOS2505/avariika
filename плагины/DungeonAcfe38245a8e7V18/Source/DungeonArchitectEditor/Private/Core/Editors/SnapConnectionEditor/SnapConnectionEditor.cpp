...
void FSnapConnectionEditor::InitSnapConnectionEditor(const EToolkitMode::Type Mode,
    ...
    ThemeGraphHandler->Initialize(GraphEditor, DetailsPanel, ThemeGraph);
    ThemeGraphHandler->SetEditorPropertiesCallback([AssetBeingEdited = AssetBeingEdited]() {
...
void FSnapConnectionEditor::PostLoadInitThemeGraph() const {
    UEdGraph_DungeonTheme* ThemeGraph = Cast<UEdGraph_DungeonTheme>(AssetBeingEdited->ThemeAsset->UpdateGraph);
    if (!ThemeGraph) return;

    for (UEdGraphNode* Node : ThemeGraph->Nodes) {
        if (UEdGraphNode_DungeonThemeBase* DungeonNode = Cast<UEdGraphNode_DungeonThemeBase>(Node)) {
...
void FSnapConnectionEditor::OnNodeSelectionChanged(const TSet<UObject*>& NewSelection) {
    ...
    for (UObject* Object : NewSelection) {
        if (UEdGraphNode_DungeonThemeMarker* MarkerNode = Cast<UEdGraphNode_DungeonThemeMarker>(Object)) {
...
namespace {
    void CreateThemeNodeUnderMarker(const FSnapConnectionVisualInfo_DEPRECATED& VisualInfo, UEdGraphNode_DungeonThemeMarker* MarkerNode) {
        if (!MarkerNode) return;
        UEdGraph_DungeonTheme* ThemeGraph = Cast<UEdGraph_DungeonTheme>(MarkerNode->GetGraph());
        if (!ThemeGraph) return;
        ...
        if (VisualInfo.bStaticMesh) {
            UEdGraphNode_DungeonThemeMesh* MeshNode = ThemeGraph->CreateNewNode<UEdGraphNode_DungeonThemeMesh>(NodePosition);
            ...
            UEdGraphNode_DungeonThemeActorTemplate* ActorNode = ThemeGraph->CreateNewNode<UEdGraphNode_DungeonThemeActorTemplate>(NodePosition);
...
void FSnapConnectionEditor::CompileAsset() const {
    if (AssetBeingEdited && AssetBeingEdited->ThemeAsset) {
        TArray<FDungeonThemeGraphBuildError> CompileErrors;
        FDungeonArchitectThemeEditorUtils::CompileThemeGraph(AssetBeingEdited->ThemeAsset, CompileErrors);
    }
}
...
FVector2D FSnapConnectionEditor::GetAssetDropGridLocation() const {
    ...
    const FVector2D GridLocation = PanelCoord / ZoomAmount + FVector2D(ViewLocation);
...
UEdGraph* FSnapConnectionEditor::CreateNewThemeGraph() const {
    UEdGraph_DungeonTheme* NewGraph = NewObject<UEdGraph_DungeonTheme>(
