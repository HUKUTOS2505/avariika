...
static UEdGraphNode_DungeonThemeActorBase* GetGraphNodeFromDetailsObject(UObject* InObject) {
    return Cast<UEdGraphNode_DungeonThemeActorBase>(InObject);
}
...
void FDATransformDetails::CacheTransform() {
    FVector CurLoc = FVector::Zero();
    FRotator CurRot = FRotator::ZeroRotator;
    FVector CurScale = FVector::One();

    for (int32 ObjectIndex = 0; ObjectIndex < SelectedObjects.Num(); ++ObjectIndex) {
        TWeakObjectPtr<UObject> ObjectPtr = SelectedObjects[ObjectIndex];
        if (ObjectPtr.IsValid()) {
            UObject* Object = ObjectPtr.Get();
            UEdGraphNode_DungeonThemeActorBase* GraphNode = Cast<UEdGraphNode_DungeonThemeActorBase>(Object);
...
void FDATransformDetails::OnSetLocation(float NewValue, ETextCommit::Type CommitInfo, int32 Axis) {
    ...
    for (int32 ObjectIndex = 0; ObjectIndex < SelectedObjects.Num(); ++ObjectIndex) {
        ...
        if (ObjectPtr.IsValid()) {
            UObject* Object = ObjectPtr.Get();
            UEdGraphNode_DungeonThemeActorBase* Node = GetGraphNodeFromDetailsObject(Object);
            ...
            if (Node) {
                ...
                if (OldValue != NewValue) {
                    ...
                    FProperty* OffsetProperty = PropertyAccessUtil::FindPropertyByName(
                        "Offset", UEdGraphNode_DungeonThemeActorBase::StaticClass());
                ...
                if (OldValue != NewValue) {
                    ...
                    if (!bPreserveScaleRatio) {
                        UStruct* VectorStruct = FindObjectChecked<UScriptStruct>(
                            UObject::StaticClass()->GetOutermost(), TEXT("Vector"), EFindObjectFlags::ExactClass);
