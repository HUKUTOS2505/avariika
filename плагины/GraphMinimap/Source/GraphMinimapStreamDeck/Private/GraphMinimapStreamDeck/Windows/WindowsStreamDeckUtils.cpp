...
#include "GraphMinimapStreamDeck/Windows/WindowsStreamDeckUtils.h"
...
namespace GraphMinimap
{
	namespace WindowsStreamDeckDefine
	{
		...
		static const FString StreamDeckPluginName = TEXT("com.naotsun.graphminimap.sdPlugin");

		// The name of the Stream Deck plugin package.
		static const FString StreamDeckPluginPackageName = TEXT("com.naotsun.graphminimap.streamDeckPlugin");
	}
	...
	void FWindowsStreamDeckUtils::InstallStreamDeckPlugin()
	{
		...
		const FString StreamDeckPluginFilePath = FPaths::ConvertRelativePathToFull(
			Plugin->GetBaseDir() / TEXT("Resources") / TEXT("StreamDeck") / WindowsStreamDeckDefine::StreamDeckPluginPackageName
		);
